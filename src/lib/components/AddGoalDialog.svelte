...
{#if isOpen}
  ...
  >
    <div class="bg-[#1D2023] border border-white/[0.12] rounded-2xl p-12 w-[900px] max-h-[90vh] overflow-y-auto">
      <!-- 헤더 -->
      <div class="mb-12">
        <h2 id="add-goal-title" class="text-white text-3xl font-bold">목표 추가</h2>
      </div>
      ...
      <div class="grid grid-cols-2 gap-16">
        ...
        <div class="space-y-8">
          <!-- 반복 일정 -->
          <div class="space-y-3">
            <span id="weekday-group-label" class="block text-white text-lg font-medium">반복 일정</span>
            <div class="flex gap-3" role="group" aria-labelledby="weekday-group-label">
              {#each [
                { id: 'mon', label: '월' },
                { id: 'tue', label: '화' },
                { id: 'wed', label: '수' },
